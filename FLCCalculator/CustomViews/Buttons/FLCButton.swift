...
final class FLCButton: UIButton {
    ...
    convenience init(color: UIColor, title: String, subtitle: String? = nil, systemImage: UIImage? = nil, isEnabled: Bool = true) {
        self.init(frame: .zero)
        set(color: color, title: title, subtitle: subtitle, systemImage: systemImage, isEnabled: isEnabled)
    }
    ...
    private func set(color: UIColor, title: String, subtitle: String?, systemImage: UIImage?, titleFontSize: CGFloat = 20, isEnabled: Bool) {
        ...
        if let imageName = systemImage { configuration?.image = imageName }
