...
class RegistrationVC: UIViewController {
    ...
    private let registrationButton = FLCButton(color: .flcGray.makeDarker(), title: "Зарегистрироваться", subtitle: "создать новый аккаунт")
    ...
    private func configureSignInButton() {
        ...
        let widthConstraint = signInButton.widthAnchor.constraint(equalTo: authButtonsContainer.widthAnchor, multiplier: 0.8)
        ...
        NSLayoutConstraint.activate([
            ...
            signInButton.widthAnchor.constraint(lessThanOrEqualToConstant: 350)
        ])
    }
    ...
    private func configureRegistrationButton() {
        ...
        let widthConstraint = registrationButton.widthAnchor.constraint(equalTo: authButtonsContainer.widthAnchor, multiplier: 0.8)
        ...
        NSLayoutConstraint.activate([
            ...
            registrationButton.widthAnchor.constraint(lessThanOrEqualToConstant: 350)
        ])
    }
    ...
    func moveFLCLogoImageViewUp() {
        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
            ...
            UIView.animate(withDuration: 0.5, animations: { self.view.layoutIfNeeded() }) { _ in
                self.authButtonsContainer.show(withAnimationDuration: 0.6)
            }
        }
    }
}
...
extension RegistrationVC: FLCButtonDelegate {
    func didTapButton(_ button: FLCButton) {
        switch button {
        case signInButton: self.dismiss(animated: true)
